...
async def async_matchlist():
    async with aiohttp.ClientSession(headers=header) as session:
        tasks = []
        for player in playerList:
            task = asyncio.create_task(get_matchList(session, player))
            tasks.append(task)
        await asyncio.gather(*tasks)
...
async def async_champsplayed():
    async with aiohttp.ClientSession(headers=header) as session:
        tasks = []
        for player in playerList:
            task = asyncio.create_task(get_champsPLayed(session, player))
            tasks.append(task)
        await asyncio.gather(*tasks)
...
def get_champdb():
    ...
    try:
        ...
        c.execute(
            "CREATE TABLE IF NOT EXISTS champions (champ_id text PRIMARY KEY, champ_cost integer, champ_count integer)")
        with open('champions.json', 'r') as content:
            championslist = json.load(content)
        for champ in championslist:
            # print(i['championId'],i['cost'])
            c.execute("INSERT INTO champions VALUES(?, ?, ?)",
                      (champ['championId'], champ['cost'], 0,))
        conn.commit()
    conn.close()
...
def main_program(playerList):
    ...
    asyncio.get_event_loop().run_until_complete(async_puuid())
    asyncio.get_event_loop().run_until_complete(async_matchlist())
    asyncio.get_event_loop().run_until_complete(async_champsplayed())
